...
#video hasta aca el 6 corregido 71 hacia el 144
            except mysql.connector.Error as descripcionError:
                print("¡No se conectó!",descripcionError)
    ...
    def modificar_album(self,album):
        if self.conexion.is_connected():
            try:
                cursor = self.conexion.cursor()
                sentenciaSQL = "UPDATE album SET cod_album =%s, nombre= %s, id_interprete = %s, id_genero = %s, cant_temas = %s, id_discografica = %s, id_formato = %s, fech_lanzamiento = %s, precio = %s, cantidad = %s, caratula = %s);"
                data = (
                album.getCod_album(),
                album.getNombre(),
                album.getId_interprete(),
                ...
                self.conexion.commit()
                self.conexion.close()
                print("Album modificado correctamente")
